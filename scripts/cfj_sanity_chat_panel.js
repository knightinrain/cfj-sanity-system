...
async function renderSanityPanel() {
  ...
  const content = isGM ? gmPanelContent() : playerPanelContent();
  await ChatMessage.create({
    speaker: { alias: "理智系统" },
    whisper: ChatMessage.getWhisperRecipients(isGM ? "GM" : game.user.name),
    content
  });
}

function gmPanelContent() {
  return `<div class="cfj-sanity-card"><h3>理智系统控制台</h3><p>外置宏和场景工具栏按钮已经收起。需要操作时，在聊天框输入 <code>/理智</code> 打开本卡。</p><div class="cfj-sanity-actions"><button type="button" data-cfj-sanity-action="request">发起理智判定</button><button type="button" data-cfj-sanity-action="setup">初始化或刷新选中角色</button></div><p class="cfj-sanity-note">玩家正常只点聊天卡中的判定按钮，或点角色卡上的 SAN 检定 / SAN 豁免；DC 仍由 GM 设置，玩家不会在弹窗里编辑 DC。</p></div>`;
}

function playerPanelContent() {
  return `<div class="cfj-sanity-card"><h3>理智系统</h3><p>跑团中等待 GM 发起理智判定。GM 发起后，这里会出现可点击的理智检定或理智豁免按钮。</p><div class="cfj-sanity-actions"><button type="button" data-cfj-sanity-action="setup">初始化或刷新我的角色</button></div><p class="cfj-sanity-note">建卡时可初始化一次；正常游戏中不需要反复处理。</p></div>`;
}
...
function requestDialogFromChat() {
  ...
  new Dialog({
    ...
    buttons: {
      ok: { label: "发送到聊天卡", callback: async (html) => {
