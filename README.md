...
模块不再要求把多个宏按钮摆在外面。GM 的聊天输入框上方会出现一整行 **理智系统** 按钮；玩家端不会显示这个按钮。GM 点开后的控制台、DC、同源、熟练、主动深入和目标选择都只对 GM 可见。DM 发起后，玩家只会收到自己的判定卡。
...
1. 在聊天输入框上方点击 **理智系统**。
2. 聊天区会出现只对 GM 可见的 **理智系统控制台**。
3. 点击 **发起理智判定**。
4. 设置类型、DC、同源、是否加入熟练、是否主动深入，并勾选目标角色。
5. 发送后，每个目标角色的拥有者会收到自己的判定卡；GM 也能看到这些卡。
